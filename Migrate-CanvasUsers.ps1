...
foreach ($student in $studentInfoData) {
    ...
    # SQL Query to Update pseudonyms table
    $updateQuery = "UPDATE pseudonyms SET unique_id = @samAccountName WHERE unique_id = @accountId"
    $updateCommand = $npgsqlConnection.CreateCommand()
    $updateCommand.CommandText = $updateQuery
    $updateCommand.Parameters.Add((New-Object Npgsql.NpgsqlParameter("@samAccountName", $samAccountName))) | Out-Null
    $updateCommand.Parameters.Add((New-Object Npgsql.NpgsqlParameter("@accountId", $accountId))) | Out-Null

    try {
        $updateCommand.ExecuteNonQuery()
        Log-Message -Message "Updated Canvas pseudonyms record for unique_id: $accountId to: $samAccountName" -Level "INFO"
    } catch {
        Log-Message -Message "Failed to update pseudonyms for account_id $($accountId): $(${_.Exception.Message})" -Level "ERROR"
        continue
    }
